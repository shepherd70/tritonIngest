...
**Status:** Implemented (v0.5.0) — Phase 0–1 complete, plus an added
...
1.  **Pure functions, no side effects** (except the explicit profile
    read/write). Cold-session unit-testable — both repos already hold
    this line.
...
4.  **Backward-compatible migration.** Each repo keeps its current
